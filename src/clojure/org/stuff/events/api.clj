...
(defn build-forecast-url [postal-code]
  (str "http://api.openweathermap.org/data/2.5/weather?zip=" postal-code ",us&units=imperial"))

(defn fetch-forecast [postal-code]
  (let [url (build-forecast-url postal-code)
        resp (http-get (get-http-client) {:url url})]
    (:body resp)))
