...
(defn create-dialog
  ...
  (let [builder (android.app.AlertDialog$Builder. context)]
    (.setPositiveButton builder "Submit" nil)
    (.setNegativeButton builder "Cancel" nil)
...
(defn show-dialog! [^Activity context ^String message ^View view buttons]
...
(defn show-input-dialog [ctx cb]
  (show-dialog!
    ctx
    "Clojure Stormy"
    (ui/make-ui ctx [:linear-layout {:orientation :vertical,
                                     :id-holder true}
                                    [:edit-text {:hint "Enter Your Postal Code"
                                                 :id ::postal-code
                                                 :input-type InputType/TYPE_CLASS_NUMBER}]
                                    ])
    {:negative-name "Cancel"
     :negative-func cancel
     :positive-name "Submit"
     :positive-func (fn [ctx v btn]
                      (let [postal-code (.getText (find-view v ::postal-code))]
                        (cb postal-code))
                        true)}))
